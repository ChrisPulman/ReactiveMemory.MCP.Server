...
/// <summary>Provides VectorStoreTests behavior.</summary>
public class VectorStoreTests
{
    /// <summary>Maximum number of vector results requested by the filter fixture.</summary>
    private const int VectorQueryLimit = 5;

    /// <summary>Executes the Vector_Query_Returns_Filtered_Sector_And_Vault_Hits operation.</summary>
    ...
    public async Task Vector_Query_Returns_Filtered_Sector_And_Vault_Hits()
    {
        ...
        var store = new JsonVectorStore(options, new SimpleTextEmbeddingProvider());
        ...
        var result = await store.QueryAsync("authentication tokens", VectorQueryLimit, new Dictionary<string, string?> { ["sector"] = "project" });
